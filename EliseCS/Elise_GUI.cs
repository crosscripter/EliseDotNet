...
{	
	...
	void DrawUI()
	{
		...
		/** Left Panel **/
		/* Search Options */
		var sources = Directory.GetFiles("Sources", "*.src").Select(p => Path.GetFileNameWithoutExtension(p)).ToArray();
	...
	{
		Formatter formatter = Formatter.GetFormatterByLanguage(Language);
		...
		GridRenderer renderer = GridRenderer.GetRendererByLanguage(Language, sequencer.Grid, hits.ToArray());
		RTF = renderer.Render();
		Grid.Rtf = RTF;
